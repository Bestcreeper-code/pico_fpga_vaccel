module qserial_interface #(
    parameter int WORD_BITS = 32
) (
    input  logic clk_in,
    input  logic rst,
    input  logic is_rx,
    inout  logic [3:0] serial_data_bus,
    input  logic [WORD_BITS-1:0] tx_word,
    output logic [WORD_BITS-1:0] rx_word,
    output logic new_word
);
    localparam int CHUNKS = WORD_BITS / 4;
    localparam int CNT_W = $clog2(CHUNKS);
    logic [CNT_W-1:0] shift_count;
    logic [WORD_BITS-1:0] rx_shift;

    always_ff @(posedge clk_in or negedge rst) begin
        if (!rst) begin
            shift_count <= '0;
            rx_shift    <= '0;
            rx_word     <= '0;

            new_word     <= 1'b0;
        end else if (shift_count < CHUNKS-1) begin
            if (is_rx) begin
                rx_shift    <= {rx_shift[WORD_BITS-5:0], serial_data_bus};
                shift_count <= shift_count + 1;
            end else begin

            end

            new_word <= 1'b0;
        end else begin
            rx_word     <= {rx_shift[WORD_BITS-5:0], serial_data_bus};
            shift_count <= '0;

            new_word     <= 1'b1;
        end
    end
endmodule